...
# ========== КОНФИГУРАЦИЯ ==========
# Получение токена ТОЛЬКО из переменных окружения Railway
BOT_TOKEN = os.environ.get('BOT_TOKEN')

# Админы (только из переменных окружения)
...
def check_balance_command(message):
    ...
    # Проверяем разные варианты команды баланса
    balance_commands = ['б', 'b', 'баланс', 'balance']
...
📋 *Примеры:*
• `50 1 7` - 50 на число 7 в рулетке
• `б мины 100 5` - 100 на мины (5 мин)
"""
...
def handle_all_messages(message):
    ...
    # 1. Проверка баланса (б, b, баланс)
    if check_balance_command(message):
        return
    
    # 2. Проверяем мины (б мины ...)
    if message.text.lower().startswith('б мины'):
        if process_mines_bet(message):
            return
    
    # 3. Пробуем обработать как ставку на рулетку (формат: 50 1 7)
    try:
    ...
    # 4. Если не распознано - показываем помощь
    help_text = """
❓ *Неизвестная команда*
