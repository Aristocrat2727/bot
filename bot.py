...
def process_mines_click(call, game_id, cell_index):
    ...
    try:
        logger.info(f"Ищем игру: {game_id}")
        logger.info(f"Доступные игры: {list(mines_games.keys())}")
...
ℹ️ Для списка игр используй /help
"""
...
⚡ *БЫСТРЫЙ СТАРТ:*
1. `/start` - начало
2. `б` - проверить баланс
3. `б мины 50` - играть в мины
4. `100 red` - играть в рулетку
