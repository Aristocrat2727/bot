...
MINES_COUNT = 5
GRID_SIZE = 5
DAILY_BONUS_AMOUNT = 2500  # Фиксированный бонус 2500 GRAM
...
def get_mines_multiplier(opened_cells):
    ...
    multipliers = [
        1.00, 1.28, 1.65, 2.10, 2.65, 3.30, 4.05, 5.00, 6.15, 7.50,
...
def check_balance_command(message):
    ...
    if text in balance_commands:
        ...
        # Создаем инлайн-клавиатуру для бонуса
        keyboard = [
            [InlineKeyboardButton(
                text="🎁 Получить ежедневный бонус 2500 GRAM", 
                url=f"https://t.me/{bot.get_me().username}?start=bonus"
            )]
        ]
        markup = InlineKeyboardMarkup(keyboard)
...
# ========== ОБРАБОТЧИК БОНУСА ==========
def process_bonus_command(message):
    """Обработка команды бонус на русском"""
    text = message.text.strip().lower()
    ...
    if text == 'бонус':
        ...
        if not user:
            bot.reply_to(message, "❌ Ошибка получения данных пользователя")
            return True
        ...
        if last_bonus:
            ...
            if hours_passed < 24:
                ...
                bot.reply_to(message, f"⏳ Вы уже получали бонус сегодня. Следующий бонус через {hours_left}ч {minutes_left}мин")
                return True
        
        # Выдача бонуса 2500 GRAM
        bonus_amount = DAILY_BONUS_AMOUNT
        new_balance = update_balance(user_id, bonus_amount)
        update_last_bonus(user_id)
        add_transaction(0, user_id, bonus_amount, "daily_bonus")
...
def send_welcome(message):
    ...
    # Проверяем, если старт с параметром bonus
    if len(message.text.split()) > 1 and message.text.split()[1] == 'bonus':
        # Используем ту же логику что и для команды "бонус"
        process_bonus_command(message)
        return
...
`бонус` - получить ежедневный бонус 2500 GRAM
...
def handle_all_messages(message):
    ...
    # Проверяем команду бонус (русская)
    if process_bonus_command(message):
        return
